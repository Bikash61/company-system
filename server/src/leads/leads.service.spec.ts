...
describe('LeadsService', () => {
  ...
  const mockLeadModel = {
    ...
    find: jest.fn(() => ({ sort: jest.fn(() => ({ exec: jest.fn().mockResolvedValue([mockLead]) })) })),
    findById: jest.fn(() => ({ exec: jest.fn().mockResolvedValue(mockLead) })),
    findByIdAndUpdate: jest.fn(() => ({ exec: jest.fn().mockResolvedValue(mockLead) })),
  ...
  describe('updateStatus', () => {
    it('should update a lead status', async () => {
      ...
      (model.findByIdAndUpdate as jest.Mock).mockReturnValueOnce({ exec: jest.fn().mockResolvedValue(updatedLead) });
      
      const result = await service.updateStatus('some-id', LeadStatus.CONTACTED);
      
      expect(model.findByIdAndUpdate).toHaveBeenCalledWith('some-id', { status: LeadStatus.CONTACTED }, { new: true });
